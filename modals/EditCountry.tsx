...
const EditCountry: React.FC<EditCountryProps> = ({ countryId, onSuccess }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      try {
        const response: any = await CountriesControllers.updateCountry(countryId, values);
        ...
        if (response.data.success) {
          ...
          if (fetchedData) {
            formik.setValues({
              ...
              phoneCode: fetchedData.phoneCode || "",
              currencyCode: fetchedData.currencyCode || "",
            });
          }
        }
      } catch (error: any) {
        setSnackbar("Failed to fetch country details", "error");
        hideModal();
      } finally {
        setFetching(false);
      }
  ...
  return (
    <Box sx={{ p: 0.5 }}>
      <Box sx={{ mb: 3 }}>
        ...
        >
          Modify the configuration parameters for this country region.
        </Typography>
      </Box>
